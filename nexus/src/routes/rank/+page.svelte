<script lang="ts">
	...
	> = {
		...
		Challenger: {
			background:
				'linear-gradient(135deg, #ffffff 0%, #f8f8ff 25%, #f3eaff 50%, #eaf6ff 75%, #fff0fa 100%)',
			textColor: '#2C2C2C',
	...
	function getRankGlow(tier: string, division: number | null): string {
		...
		if (tier === 'Challenger') {
			...
			return rankStyles[tier].glow
				...
				.replace(/rgba\(([^,]+,){3}([^)]+)\)/g, (m, p1, alpha) =>
					m.replace(alpha, (parseFloat(alpha) * 0.4).toString())
				);
		}
		...
		if (tier === 'Platinum') {
			...
			return rankStyles[tier].glow
				...
				.replace(/rgba\(([^,]+,){3}([^)]+)\)/g, (m, p1, alpha) =>
					m.replace(alpha, (parseFloat(alpha) * 0.3).toString())
				);
		}
		return '';
	}
...
>
	{#if showAllRanks}
		<div class="mb-8 flex flex-col items-center">
			<div class="mb-4 text-2xl font-semibold" style="color: #CCCCCC;">All Ranks</div>
			<div class="grid max-w-4xl grid-cols-4 gap-4">
				{#each allRanks as rank}
					<div
						class="rounded-lg px-4 py-2 text-center font-semibold transition-all duration-300 hover:scale-105"
						style="
	...
	{#if showRank}
		<div class="mb-12 text-center" in:scale={{ duration: 1000, easing: elasticOut }}>
			<div class="mb-4 text-4xl font-bold" style="color: #CCCCCC;">Placements Complete!</div>
			<div
				class="inline-block rounded-2xl px-16 py-10 text-6xl font-extrabold transition-all duration-300 hover:scale-110"
				style="
				...
				"
			>
				{userRank.tier}{['Master', 'Grandmaster', 'Challenger'].includes(userRank.tier)
					? ''
					: ` ${userRank.division}`}
			</div>
		</div>
		...
		<div class="flex gap-4">
			<button
				...
				class="rounded-lg bg-blue-600 px-8 py-3 font-semibold text-white transition-colors duration-200 hover:bg-blue-700"
			>
				Create Account
			</button>
			<button
				onclick={backToNexus}
				class="rounded-lg bg-gray-600 px-8 py-3 font-semibold text-white transition-colors duration-200 hover:bg-gray-700"
			>
				Back to Nexus
			</button>
		</div>
