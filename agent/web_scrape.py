...
AVAILABLE ACTIONS
- find_paper_id: <query>
  → Returns the first arXiv id (e.g., 2506.01844v1).
- return_text: <arxiv_id>
  → Saves FULL plaintext to papers/<id>.txt; returns path. (Optional)
- return_core_text: <arxiv_id or papers/<id>.txt>
  → Returns the core sections text (Abstract + Introduction + optional Methods + Conclusion), trimmed to ~12k chars.

MUST-DO TOOL USE
- If the user gives a query (not an id), you MUST call find_paper_id, then call return_core_text with that id.
- If the user gives an arXiv id, call return_core_text with that id.
- Do NOT answer until an Observation from return_core_text contains [ABSTRACT] or [INTRODUCTION].
- If core text is unavailable, output [].

GOAL
Given the core text, produce 6–15 textbook-grade, canonical technical TOPICS that would each merit a focused lecture or textbook section. Topics must be either:
(a) central technical ideas explicitly mentioned in the paper, or
(b) clear prerequisites needed to understand those ideas.
...
- Buzzwords/meta/logistics: “consumer-grade hardware”, “open-source initiative”, “responsiveness/efficiency” (alone), “training costs”, “deployability”, “latency” (alone), “generalization” (alone), “community-driven datasets” (alone).
- Project names with no broader concept unless they are well-known baselines/datasets/benchmarks/models (e.g., keep “Meta-World”, “LIBERO”, “Atari ALE” where appropriate).

SELECTION PRIORITIES
- Prefer explicit phrases in the core text (models, methods, losses, datasets, benchmarks, tasks, math).
- Prefer foundational prerequisites when referenced (e.g., “Transformer”, “Cross-Attention”, “Imitation Learning”, “Reinforcement Learning”, “Large Language Models”, “Vision-Language Models”, “Flow Matching”, “Action Chunking”).
- Include datasets/benchmarks only by canonical names (e.g., “Arcade Learning Environment”, “Meta-World”, “LIBERO”), and only if central.
- Include objectives/losses/math if named (e.g., “Q-learning”, “Bellman Optimality Equation”, “Flow Matching”).

CANONICALIZATION STEP (before acceptance gate)
- Map surface phrasings to canonical, reusable topics. Choose the best single canonical topic; don’t list both unless both are central.
  Examples:
  • “learns directly from raw pixels” → End-to-End Reinforcement Learning (method) or Representation Learning (concept)
  • “predicts action values” / “output is a value function” → Value Function Approximation (concept)
  • “variant of Q-learning” → Q-learning (method); possibly Bellman Optimality Equation (math)
  • “convolutional neural network” → Convolutional Neural Networks (architecture)
  • “learn control policies” → Policy Learning (concept) (use if central; else covered by RL/Q-learning)
...
QUANTITY & ORDERING
- Return 0-5 topics. Do NOT pad with weak items; fewer is fine.
- Sort by confidence descending. Calibrate: explicit named items ≈ 0.9–1.0; strongly implied, prerequisite math topics ≈ 0.75–0.9; weakly implied < 0.7 (usually exclude).

QUALITY GUARD
- If fewer than 5 candidates pass, return only those that pass (possibly <5). Never include banned/buzzy items.
...
def _normalize_topics(topics: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Coerce fields; clamp values; drop duplicates; cap to 20, prefer >=6 if present."""
    norm: List[Dict[str, Any]] = []
    seen = set()
    for t in topics:
        if not isinstance(t, dict):
...
if __name__ == "__main__":
 question = """Extract topics for 2408.00714."""
 query(question)
